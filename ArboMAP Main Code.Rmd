...
humandatafile <- "C:\\home\\work\\davis\\WNV repository\\South Dakota\\human\\20-07-15 - SD human WNV.csv"

# set up dates to constrain the human, mosquito, and weather data
humanbelievableyears    <- c(2004:2019)
mosquitobelievableyears <- c(2004:2020)
...
weatherpathstr <- "C:\\home\\work\\davis\\WNV repository\\South Dakota\\weather\\"
...
mosqfile <- "C:\\home\\work\\davis\\WNV repository\\South Dakota\\mosquito\\20-07-15 - mosquito testing records.csv"
...
wnv <- wnv[which(wnv$doy >= 140),]
# delete anything after a certain day
wnv <- wnv[which(wnv$doy <= 214),]
...
randeffs <- data.frame(year=as.numeric(rownames(random.effects(infectglm)$year)),
                       mosqinfect=random.effects(infectglm)$year$`(Intercept)`)
