...
modelformulas <- c("cub-fx-nonanom"= "anycases ~ 0 + district + MIRsummarystat + s(lag, by=var1, bs='cr') + s(lag, by=var2, bs='cr')",
                   "cub-fx-anom"   = "anycases ~ 0 + district + MIRsummarystat + s(lag, by=anom_var1, bs='cr') + s(lag, by=anom_var2, bs='cr') + s(doy, bs='cr')",
                   "cub-sv-nonanom"= "anycases ~ 0 + district + MIRsummarystat + te(lag, doymat, by=var1, bs='cr') + te(lag, doymat, by=var2, bs='cr')",
                   "cub-sv-anom"   = "anycases ~ 0 + district + MIRsummarystat + te(lag, doymat, by=anom_var1, bs='cr') + te(lag, doymat, by=anom_var2, bs='cr') + s(doy, bs='cr')",
                   "tp-fx-nonanom" = "anycases ~ 0 + district + MIRsummarystat + s(lag, by=var1, bs='tp') + s(lag, by=var2, bs='tp')",
                   "tp-fx-anom"    = "anycases ~ 0 + district + MIRsummarystat + s(lag, by=anom_var1, bs='tp') + s(lag, by=anom_var2, bs='tp') + s(doy, bs='tp')",
...
# modelnames <- c("cub-fx-nonanom",
#                 "tp-sv-anom")

# where are the human data located?
# example of a file with directories an drive:
humandatafile <- "C:\\home\\work\\davis\\WNV repository\\Louisiana\\human\\20-07-15 - LA human data.csv"

# set up dates to constrain the human, mosquito, and weather data
humanbelievableyears    <- c(2004:2019)
mosquitobelievableyears <- c(2012,2014:2020)
envbelievableyears      <- 2000:2020
...
weekinquestion <- as.Date("2020-07-27", "%Y-%m-%d")

# where are the weather csv files stored?
weatherpathstr <- "C:\\home\\work\\davis\\WNV repository\\Louisiana\\weather\\"
...
mosqfile <- "C:\\home\\work\\davis\\WNV repository\\Louisiana\\mosquito\\19-08-29 - reconciled mosquito data.csv"
...
The table below shows the relative risk per year according to the mosquito data.

```{r mosquitodataprocess2, fig.width=7, fig.height=3, echo=FALSE, include=FALSE}
randeffs <- wnv
randeffs$mosqinfect <- randeffs$numpos / randeffs$total
...
randeffs <- randeffs[c("year", "mosqinfect")]
kable(randeffs,
      digits = 3,
      col.names=c("year", "mosq risk"),
      align="c")
```
...
yearfiller$MIRsummarystat[is.na(yearfiller$MIRsummarystat)] <- mean(yearfiller$MIRsummarystat, na.rm=TRUE)
...
tempdf2 <- preds[preds$year == as.numeric(format(weekinquestion, "%Y")),]
...
thisplot2 <- ggplot(data=tempdf2) +
  ...
  scale_color_manual("", values = mycolors) +
  scale_linetype_manual("", values = mylinetypes) +
  ggtitle(paste("Statewide model predictions in ", as.numeric(format(weekinquestion, "%Y")), sep="")) +
  xlab("") + ylab("") +
...
relationship <- lm(totcases ~ poly(totpos, 2), weights=weight, data=prevyears)
...
district_shapes <- counties(state=statetomodel, cb=TRUE)
district_shapes$district <- simplifynames(district_shapes$NAME)
# district_shapes$id <- district_shapes$NAME
# projected_districts.df <- broom::tidy(district_shapes)
...
for (curmodel in unique(thisweek$model)) {
  ...
  thisplot <- ggplot(tempdf) +
      geom_sf(aes(fill=pred)) + xlab("") + ylab("") +
      theme(legend.position="bottom") +
      scale_fill_distiller(palette = "Spectral", limits=c(0,1),
                                                 ...
                                                 labels = c("Will definitely not\nreport any cases",
                                                            "Will definitely\nreport some cases"),
                                                     name = "") +
      ggtitle(paste("Estimate for week beginning ",
                    weekinquestionSun,
                    " (", curmodel, ")",
                    sep="")) +
...
projected_districts.df <- left_join(district_shapes, doypredscurrent,
                                   by="district")
projected_districts.df$riskcategory[is.na(projected_districts.df$riskcategory)] <- " About average    "
