...
humandatafile <- ".\\human case data\\updated michigan human case data.csv"

# set up dates to constrain the human, mosquito, and weather data
humanbelievableyears    <- c(2004:2018)
mosquitobelievableyears <- c(2004:2018)
envbelievableyears      <- 2000:2018
...
mosqfile <- ".\\mosquito data\\updated michigan mosquito data.csv"
...
wnv$district <- simplifynames(wnv$county)
...
# calculate simple metric of infection
wnv$indicator <- 1*(wnv$WNV == "POS")
wnv <- group_by(wnv, year)
randeffs <- dplyr::summarise(wnv,
                             totpos=sum(indicator, na.rm=TRUE),
                             tottest=n())
randeffs$mosqinfect <- randeffs$totpos / randeffs$tottest
```
...
begrow <- nrow(human)
human$chardate <- as.character(human$onsetdate)
#human$date <- as.Date(human$onsetdate, "%m/%d/%Y")
human$date <- as.Date(human$onsetdate, "%d-%b-%y")
...
human$district <- simplifynames(human$county)
...
names(randeffs) <- c("year","num", "denom", "MIRsummarystat")
randeffs <- randeffs[c("year","MIRsummarystat")]
